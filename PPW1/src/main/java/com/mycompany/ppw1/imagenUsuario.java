...
public class imagenUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement comando = conexion.prepareStatement("SELECT imagen FROM t_usuario WHERE idUsuario = ?");
            comando.setInt(1, idNum);
            if (comando.execute()) {
                ResultSet miSelect = comando.getResultSet();
                miSelect.next();
                byte[] imagen = miSelect.getBytes("imagen");
                response.getOutputStream().write(imagen);
            }
            comando.close();
            conexion.close();
        } catch (NamingException | SQLException ex) {
            Logger.getLogger(crearNota.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
