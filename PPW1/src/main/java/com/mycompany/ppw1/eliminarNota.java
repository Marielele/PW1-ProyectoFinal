...
public class eliminarNota extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement comando = conexion.prepareStatement("UPDATE t_nota SET activa = 0 WHERE id = ?");
            comando.setInt(1, idN);
            comando.execute();

        } catch (NamingException | SQLException ex) {
            Logger.getLogger(eliminarNota.class.getName()).log(Level.SEVERE, null, ex);
        }
